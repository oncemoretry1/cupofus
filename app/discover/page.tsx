...
export default function DiscoverPage(){
  ...
  return <main className="catalog-page"><nav className="catalog-nav"><a className="logo" href="/"><span>CUP</span><i>of</i><span>US</span></a><div><a href="/brew">Brew yours</a><a href="/club">Cup (Club)</a><a href="/partners">Near me</a><a className="profile-icon" href="/profile" aria-label="โปรไฟล์">●</a></div></nav>
    ...
    <section className="catalog-grid">{shown.map((book,index)=>{
      ...
      return <article className="catalog-card" key={book.slug} style={{"--cover":book.coverColor,"--cover-ink":coverInk,"--coffee-top":coffee?.layerTop??book.coverColor,"--coffee-bottom":coffee?.layerBottom??"#6f4937"} as React.CSSProperties}>
        ...
        <div className="catalog-copy"><p>{book.tags.split(",").slice(0,2).join(" · ")}</p><a className="catalog-title-link" href={`/cup/${book.slug}`}><h2>{book.title}</h2><h3>{book.thaiTitle}</h3></a><span>{book.summary}</span><div className="catalog-coffee"><i></i><div><small>แก้วคู่หนังสือ · {coffee?.menuNameThai??"เมนูประจำเล่ม"}</small><b>{coffee?.menuName??"Cup of Us Blend"}</b></div></div></div>
        <footer className="catalog-actions" aria-label={`เลือกสิ่งที่อยากทำกับ ${book.title}`}><a href={`/read/${book.slug}`}><i>AA</i><span><b>อ่าน</b><small>บทสรุป 10–15 นาที</small></span></a><a href={`/cup/${book.slug}#listen`}><i>▶</i><span><b>ฟัง</b><small>เสียงและคลิปสรุป</small></span></a><a href={`/cup/${book.slug}#buy`}><i>▣</i><span><b>ซื้อ</b><small>ดูร้านค้าที่มีเล่มนี้</small></span></a></footer>
      </article>
    })}</section>
