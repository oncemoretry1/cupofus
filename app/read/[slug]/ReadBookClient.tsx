...
export default function ReadBookClient({ book }: { book: ReadBook }) {
  ...
  useEffect(() => {
    fetch(
      `/api/external/books?title=${encodeURIComponent(book.title)}&author=${encodeURIComponent(book.author)}`,
    ).then((response) => response.json()).then((result) => {
      setVolume(result.volume ?? null);
      setPreviewFallback(result.fallbackUrl ?? "");
    }).catch(() => {
      setVolume(null);
      setPreviewFallback(`https://books.google.com/books?q=${encodeURIComponent(`${book.title} ${book.author}`)}`);
    });
